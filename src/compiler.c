...
static void errorAt(Token *token, const char message[]) {
  ...
  fprintf(stderr, "%s:%i:%i: Error", token->position.filename,
...
static void emitByte(uint8_t byte) {
  writeChunk(currentChunk(), byte, parser.previous.position);
}
...
static void initCompiler(Compiler *compiler, FunctionType type) {
  ...
  if (type != TYPE_SCRIPT) {
    current->function->name =
        copyString(parser.previous.start, parser.previous.lenght);
  }
...
static uint8_t indentifierConstant(Token *name) {
  return makeConstant(OBJ_VAL(copyString(name->start, name->lenght)));
}
...
static void dot(bool canAssign) {
  ...
  uint8_t name = indentifierConstant(&parser.previous);
...
static void string(bool canAssign) {
  // TODO: maybe translate escape sequences here ?
  emitConstant(OBJ_VAL(
      copyString(parser.previous.start + 1, parser.previous.lenght - 2)));
}
...
static void method() {
  ...
  uint8_t constant = indentifierConstant(&parser.previous);

  FunctionType type = TYPE_METHOD;
  if (parser.previous.lenght == 4 &&
...
static void classDeclaration() {
  ...
  uint8_t nameConstant = indentifierConstant(&parser.previous);
  declareVariable();
...
ObjFunction *compile(const char source[], const char filename[],
                     bool repl_mode) {
  initScanner(source, filename, repl_mode);
  Compiler compiler;
  ...
  while (!match(TOKEN_EOF)) {
    ...
    while (match(TOKEN_SEMICOLON)) {
      // eats semicolon when there is too many
    }
  }
