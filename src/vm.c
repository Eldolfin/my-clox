...
static void runtimeError(const char format[], ...) {
  ...
  for (int i = vm.frameCount - 1; i >= 0; i--) {
    ...
    Position p = function->chunk.positions[instruction];
    fprintf(stderr, "%d:%d:%s in ", p.column, p.line, p.filename);
...
#define BINARY_OP_BREAK(typeValue, op)                                         \
  ...
  while (true) {
    ...
    case OP_SET_PROPERTY: {
      ...
      tableSet(&instance->fields, READ_STRING(), value);
      push(value);
      break;
    }
    case OP_GET_SUPER: {
...
InterpretResult interpret(const char source[], const char filename[]) {
  ObjFunction *function = compile(source, filename, vm.repl_mode);
  if (function == NULL)
